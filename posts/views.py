...
class NewsFeedView(ListAPIView):
    ...
    def get_queryset(self):
        # Optional: personalized feed (posts from followed users)
...
GOOGLE_DRIVE_CREDENTIALS = "C:/Users/jhuns/Downloads/apt-api-group10/service_account.json"
...
class PostListCreate(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        if not post_type:
            raise serializers.ValidationError({'error': 'Missing post_type'})
...
# -------------------- GOOGLE OAUTH --------------------
class GoogleLogin(SocialLoginView):         # This view will handle the Google OAuth login process
...
class ConvertTokenView(APIView):
    ...
    def post(self, request):
        ...
        # Get or create user, assigning default password 'mmdc2026' if new
        user_model = get_user_model()
        user, created = user_model.objects.get_or_create(
            email=email, 
            defaults={"username": email.split("@")[0], "password": make_password("mmdc2026")}
        )
